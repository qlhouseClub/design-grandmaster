...
需要多个兼容 Agent 共享时，可以使用 `~/.agents/skills/design-grandmaster/`。
...
重启 ChatGPT 桌面端，在 Plugins 中安装“设计宗师”。需要团队使用时，可从插件详情页分享到 ChatGPT 工作区。生成的单插件压缩包位于 `dist/openai/design-grandmaster-plugin.zip`。
...
在 TRAE 的“设置 → 技能与命令/规则和技能 → 创建或上传技能”中选择 `dist/trae/design-grandmaster.zip`。压缩包采用 `design-grandmaster/SKILL.md` 的目录包裹结构。
...
```text
%USERPROFILE%/.trae-cn/skills/design-grandmaster/
```
...
```powershell
git clone https://github.com/qlhouseClub/design-grandmaster.git "$env:USERPROFILE\.hermes\skills\design-grandmaster"
```

也可以生成 `dist/portable/design-grandmaster.zip`，解压到 `~/.hermes/skills/`。仓库是私有仓库时，在线安装需要有效的 GitHub 凭据或 Token。
...
2. 新建知识库，上传 `dist/coze/design-grandmaster/knowledge/` 中全部 Markdown 文件。
3. 把知识库绑定到智能体并启用检索。
4. 需要实时视觉趋势、在线资料、Figma 或代码库操作时，再配置相应插件或工作流；没有工具时不得假装完成实时调研或外部操作。
5. 用现有规范项目、完整应用和高视觉项目分别测试 Token 遵循、研究深度、审美裁决与输出稳定性。
